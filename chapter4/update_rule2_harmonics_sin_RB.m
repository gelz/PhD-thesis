function  y = update_rule2_harmonics_sin_RB(x_p,i,seita_0)

%         x_p(k,:,:) = update_rule1_harmonics(x_p(k,:,:),i,seita_0);
% var_period = 2;
% var_amplitude = 0.05;
...
Period_num = length(find(x_p(1,:,end)~=0));
k =1;
x_p(k,Period_num+1,end) = round(x_p(k,Period_num,end)  + seita_0(1)*2*(rand(1)-0.5));
                    if  x_p(k,Period_num+1,end)<40 
                        x_p(k,Period_num+1,end) = 40;
                    elseif x_p(k,Period_num+1,end)>200
                        x_p(k,Period_num+1,end) = 200;
                    end

                    
                    
                    
% x_p(k,Period_num+1,1:end-1) = x_p(k,Period_num,1:end-1);
% x_p(k,Period_num+1,(p_order+1):(p_order+K_order)) = x_p(k,Period_num,(p_order+1):(p_order+K_order))  + seita_0(3)*randn(1);
